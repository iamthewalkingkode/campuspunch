...
class FocPhoto extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            loading: true, loadingSchools: true, applied: false,
            interval: null
        };
    }
    ...
    componentDidMount() {
        ...
        const { id } = this.props.match.params; console.log(this.props.match);
        const { auth: { logg } } = this.props;
        func.post('foc', { id: parseInt(id), status: 1, limit: 1 }).then(res => {
            this.setState({ loading: false });
            if (res.status === 200) {
                let row = res.result[0];
                this.setState({ row });

                func.post('foc/photoschool').then(res => {
                ...
                func.post('foc/users', { user: logg.id, constest: parseInt(id) }).then(res => {
    ...
    render() {
        const { data, row, loading, loadingSchools, applied, cd } = this.state;
            ...
            <React.Fragment>
                ...
                {loading === false && (
                    <div>
                        ...
                        {loadingSchools === true && (<Loading text="loading schools ..." />)}
